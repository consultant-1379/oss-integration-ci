...
pipeline {
    ...
    parameters {
        ...
        string(name: 'RESOURCE_TYPE',
               description: 'The type of the resource, \"secret\" or \"configmap\"' )
        string(name: 'RESOURCE_NAME',
            description: 'The name of the resource, secret or config map name created in the namespace' )
        string(name: 'SEARCH_STRING',
               description: 'The name of the key from the secret or configmap its data will be returned' )
        string(name: 'TIMEOUT',
            defaultValue: '3600',
        ...
        string(name: 'AGENT_LABEL',
            ...
            description: 'Specify the agent label that the Jenkins job shoulds run on')
    ...
    stages {
        ...
        stage('Get Resource Required details') {
            steps {
                retry(count: 5) {
                    script {
                        if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Get Resource Required details\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                            sleep(180)
                        }
                        else {
                            echo "Running the \"Get Resource Required details\" stage. Try ${RETRY_ATTEMPT} of 5"
                        }
                        RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                        sh "${bob} generate-secret:get_value_from_configmap_or_secret"
                        RETRY_ATTEMPT = 1
                    }
                }
            }
