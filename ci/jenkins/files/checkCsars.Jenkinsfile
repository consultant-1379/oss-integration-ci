...
pipeline {
    ...
    parameters {
        ...
        string(name: 'FULL_PATH_TO_SITE_VALUES_FILE',
                defaultValue: 'site-values/csar-check/site-values.yaml',
    ...
    stages {
        ...
        stage('Check CSARs') {
            steps {
                withCredentials([usernamePassword(credentialsId: env.FUNCTIONAL_USER_SECRET, usernameVariable: 'FUNCTIONAL_USER_USERNAME', passwordVariable: 'FUNCTIONAL_USER_PASSWORD'), file(credentialsId: params.ARMDOCKER_USER_SECRET, variable: 'DOCKERCONFIG')]) {
        ...
        always {
            script {
                if (getContext(hudson.FilePath)) {
                    archiveArtifacts artifacts: 'csar_check.properties', allowEmptyArchive: true, fingerprint: true
                }
            }
        }
        failure {
