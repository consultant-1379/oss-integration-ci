...
pipeline {
    ...
    stages {
        ...
        stage('Create VM VNFM generic secret') {
            steps {
                retry(count: 5) {
                    script {
                        if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Create VM VNFM generic secret\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                            sleep(180)
                        }
                        else {
                            echo "Running the \"Create VM VNFM generic secret\" stage. Try ${RETRY_ATTEMPT} of 5"
                        }
                        RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                        sh "${bob} generate-secret:vm-vnfm-generic-secret"
                        RETRY_ATTEMPT = 1
                    }
                }
            }
