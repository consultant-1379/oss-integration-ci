...
pipeline {
    ...
    parameters {
        ...
        string(name: 'FUNCTIONAL_USER_SECRET',
            description: 'Jenkins secret ID for a Functional user that has access to the data within DIT. ONLY Used if USE_DIT parameter is set to true')
        string(name: 'TIMEOUT',
            defaultValue: '3600',
    ...
    stages {
        ...
        stage('Create AM Specific Role Binding') {
            steps {
                retry(count: 5) {
                    script {
                        if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Create AM Specific Role Binding\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                            sleep(180)
                        }
                        else {
                            echo "Running the \"Create AM Specific Role Binding\" stage. Try ${RETRY_ATTEMPT} of 5"
                        }
                        RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                        sh "${bob} generate-am-specific-rolebinding"
                        RETRY_ATTEMPT = 1
                    }
                }
            }
