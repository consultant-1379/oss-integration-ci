...
pipeline {
    ...
    parameters {
        ...
        string(name: 'SERACC_CLSROLB_META_NAME',
            defaultValue: 'evnfm',
            description: 'Release name to be added to the file example <metadata.name <SERACC_CLSROLB_META_NAME>')
        string(name: 'USER_ID',
            defaultValue: 'vnfm',
            description: '<USER_ID> must match the <eric-eo-evnfm-nbi.eric-evnfm-rbac.defaultUser.username> in the site values file.')
        string(name: 'USER_PASSWORD',
            defaultValue: 'Ericsson123!',
            description: '<USER_PASSWORD> must match the <eric-eo-evnfm-nbi.eric-evnfm-rbac.defaultUser.password> in the site values file.')
        string(name: 'REGISTRY_SECRET_NAME',
            defaultValue: 'container-registry-users-secret',
    ...
    stages {
        ...
        stage('Update EVNFM (ClusterRoleBinding) File') {
            steps {
                sh "${bob} update-evnfm-files"
            }
        }
        ...
        stage('Create Apply ServiceAccount & ClusterRoleBinding') {
            steps {
                retry(count: 5) {
                    script {
                        if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Create Apply ServiceAccount & ClusterRoleBinding\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                            sleep(180)
                        }
                        else {
                            echo "Running the \"Create Apply ServiceAccount & ClusterRoleBinding\" stage. Try ${RETRY_ATTEMPT} of 5"
                        }
                        RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                        sh "${bob} evnfm-pre-deployment"
                        RETRY_ATTEMPT = 1
                    }
                }
            }
