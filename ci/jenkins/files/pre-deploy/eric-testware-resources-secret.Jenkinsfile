...
pipeline {
    ...
    parameters {
        ...
        string(name: 'SECRET_NAME',
            defaultValue: 'testware-resources-secret',
            description: 'The name of the Testware resource secret to be created' )
        string(name: 'TESTWARE_API_URL',
            defaultValue: 'None',
            description: 'The API URL for the Testware resource' )
        string(name: 'TESTWARE_DATABASE_URL',
            defaultValue: 'None',
            description: 'The Database URL for the Testware resource' )
        string(name: 'TESTWARE_K6_TOOL_GUI_URL',
            defaultValue: 'None',
            description: 'The GUI URL for the Testware resource' )
        string(name: 'KAFKA_CONFIG_BROKERS',
            defaultValue: 'None',
            description: 'Kafka topic brokers' )
        string(name: 'KAFKA_CONFIG_TOPIC',
            defaultValue: 'None',
            description: 'Kafka config topic based on environment' )
        string(name: 'KAFKA_CONFIG_FORMAT',
            defaultValue: 'json',
            description: 'Format of Kafka Config' )
        string(name: 'GAS_USER_USERNAME_VALUE',
               defaultValue: 'gas-user',
               description: 'Username set for the GAS User' )
        string(name: 'GAS_USER_PASSWORD_VALUE',
               defaultValue: 'Ericsson123!',
               description: 'Password set for the GAS User' )
        string(name: 'TIMEOUT',
            defaultValue: '3600',
    ...
    environment {
        ...
        USER_SECRET_USERNAME_KEY = "eic-gas-username"
        USER_SECRET_USERNAME_VALUE = "${params.GAS_USER_USERNAME_VALUE}"
        USER_SECRET_PASSWORD_KEY = "eic-gas-password"
        USER_SECRET_PASSWORD_VALUE = "${params.GAS_USER_PASSWORD_VALUE}"
        TESTWARE_KAFKA_CONFIG = set_testware_kafka_config()
    }
    stages {
        ...
        stage('Create Testware secret') {
            steps {
                retry(count: 5) {
                    script {
                        if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Create Testware secret\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                            sleep(180)
                        }
                        else {
                            echo "Running the \"Create Testware secret\" stage. Try ${RETRY_ATTEMPT} of 5"
                        }
                        RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                        sh "${bob} generate-secret:testware-resources-secret"
                        RETRY_ATTEMPT = 1
                    }
                }
            }
...
def set_testware_kafka_config() {
    return "brokers=${params.KAFKA_CONFIG_BROKERS},topic=${params.KAFKA_CONFIG_TOPIC},format=${params.KAFKA_CONFIG_FORMAT}";
}
