...
pipeline {
    ...
    parameters {
        ...
        string(name: 'SECRET_NAME',
            defaultValue: 'eric-odca-diagnostic-data-collector-sftp-credentials',
            description: 'SFTP Server Secret name to be created' )
        string(name: 'SFTP_CREDENTIALS',
            defaultValue: 'odca-ddc-sftp-credentials',
    ...
    stages {
        ...
        stage('Create SFTP Server secret with Jenkins Credentials') {
            steps {
                retry(count: 5) {
                    script {
                        withCredentials([usernamePassword(credentialsId: params.SFTP_CREDENTIALS, usernameVariable: 'SFTP_USER', passwordVariable: 'SFTP_PASSWORD')]) {
                            if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Create SFTP Server secret with Jenkins Credentials\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                                sleep(180)
                            }
                            else {
                                echo "Running the \"Create SFTP Server secret with Jenkins Credentials\" stage. Try ${RETRY_ATTEMPT} of 5"
                            }
                            RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                            sh "${bob} generate-secret:eric-odca-ddc-sftp-server-secret"
                            RETRY_ATTEMPT = 1
                        }
                    }
                }
