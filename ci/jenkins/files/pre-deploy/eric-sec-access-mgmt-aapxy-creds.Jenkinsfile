...
pipeline {
    ...
    parameters {
        ...
        string(name: 'SECRET_NAME',
            defaultValue: 'eric-sec-access-mgmt-aapxy-creds',
            description: 'Authentication Proxy Secret name to be created' )
        string(name: 'ADP_IAM_AA_CLIENT_PASSWORD',
    ...
    stages {
        ...
        stage('Create IAM Authentication Proxy secret') {
            steps {
                retry(count: 5) {
                    script {
                        if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Create IAM Authentication Proxy secret\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                            sleep(180)
                        }
                        else {
                            echo "Running the \"Create IAM Authentication Proxy secret\" stage. Try ${RETRY_ATTEMPT} of 5"
                        }
                        RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                        sh "${bob} generate-secret:eric-sec-access-mgmt-aapxy-creds"
                        RETRY_ATTEMPT = 1
                    }
                }
            }
