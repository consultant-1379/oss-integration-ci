...
pipeline {
    ...
    parameters {
        ...
        string(name: 'ENM_CONTAINER_SECRET_NAME',
            defaultValue: 'enm-secret',
            description: 'ENM secret name to be created' )
        string(name: 'ENM_SCRIPTING_CLUSTER_IP',
            description: 'ENM Scripting IP (grep -w scripting_service_IPs /ericsson/tor/data/global.properties)')
        string(name: 'ENM_USERNAME',
            defaultValue: 'enmUser',
            description: ' ENM Scripting Username')
        string(name: 'ENM_PASSWORD',
            defaultValue: 'Ericsson123!',
            description: ' ENM Scripting Username')
        string(name: 'ENM_CONNECTION_TIMEOUT_IN_MILLISECONDS',
            defaultValue: '20000',
            description: 'ENM Connection timout in milliseconds')
        string(name: 'ENM_SCRIPTING_SSH_PORT',
            defaultValue: '22',
            description: 'ENM Scripting SSH Port')
        string(name: 'TIMEOUT',
            defaultValue: '3600',
    ...
    stages {
        ...
        stage('Create Container VNFM generic secret') {
            steps {
                retry(count: 5) {
                    script {
                        if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Create Container VNFM generic secret\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                            sleep(180)
                        }
                        else {
                            echo "Running the \"Create Container VNFM generic secret\" stage. Try ${RETRY_ATTEMPT} of 5"
                        }
                        RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                        sh "${bob} generate-secret:container-vnfm-generic-secret"
                        RETRY_ATTEMPT = 1
                    }
                }
            }
