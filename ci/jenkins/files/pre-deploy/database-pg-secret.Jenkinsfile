...
pipeline {
    ...
    parameters {
        ...
        string(name: 'SECRET_NAME',
            defaultValue: 'eric-eo-database-pg-secret',
            description: 'Postgres secret name to be created' )
        string(name: 'CUSTOM_USER_ID',
            defaultValue: 'customuser',
            description: 'Postgres Custom User Id')
        string(name: 'CUSTOM_PASSWORD',
            defaultValue: 'Postgres123!',
            description: ' Postgres Custom Password')
        string(name: 'SUPER_USER_ID',
            defaultValue: 'postgres',
            description: 'Postgres Super User Id')
        string(name: 'SUPER_PASSWORD',
            defaultValue: 'Postgres123!',
            description: 'Postgres Super User Password')
        string(name: 'METRICS_USER_ID',
            defaultValue: 'exporter',
            description: 'Postgres Metric User Id')
        string(name: 'METRICS_PASSWORD',
            defaultValue: 'Postgres123!',
            description: 'Postgres Metric Password')
        string(name: 'REPLICA_USER_ID',
            defaultValue: 'replica',
            description: 'Postgres Replica User ID')
        string(name: 'REPLICA_PASSWORD',
            defaultValue: 'Postgres123!',
            description: 'Postgres Replica PASSWORD')
        string(name: 'TIMEOUT',
            defaultValue: '3600',
    ...
    stages {
        ...
        stage('Create secret database pg secret') {
            steps {
                retry(count: 5) {
                    script {
                        if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Create secret database pg secret\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                            sleep(180)
                        }
                        else {
                            echo "Running the \"Create secret database pg secret\" stage. Try ${RETRY_ATTEMPT} of 5"
                        }
                        RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                        sh "${bob} generate-secret:database-pg-secret"
                        RETRY_ATTEMPT = 1
                    }
                }
            }
