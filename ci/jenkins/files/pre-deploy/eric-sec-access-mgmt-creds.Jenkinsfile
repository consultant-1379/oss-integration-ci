...
pipeline {
    ...
    parameters {
        ...
        string(name: 'SECRET_NAME',
            defaultValue: 'eric-sec-access-mgmt-creds',
            description: 'Postgres secret name to be created' )
        string(name: 'KC_ADMIN_ID',
            defaultValue: 'admin',
            description: 'Keycloak Admin User')
        string(name: 'KC_PASSWORD',
            defaultValue: 'Ericsson123!',
            description: 'Keycloak Admin Password')
        string(name: 'PG_USER_ID',
            defaultValue: 'admin',
            description: 'Keycloak Postgres Admin Password')
        string(name: 'PG_PASSWORD',
            defaultValue: 'Ericsson-pw123!',
            description: 'Keycloak Postgres Admin Password')
        string(name: 'TIMEOUT',
            defaultValue: '3600',
    ...
    stages {
        ...
        stage('Create secret : eric-sec-access-mgmt-creds') {
            steps {
                retry(count: 5) {
                    script {
                        if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Create secret : eric-sec-access-mgmt-creds\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                            sleep(180)
                        }
                        else {
                            echo "Running the \"Create secret : eric-sec-access-mgmt-creds\" stage. Try ${RETRY_ATTEMPT} of 5"
                        }
                        RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                        sh "${bob} generate-secret:eric-sec-access-mgmt-creds"
                        RETRY_ATTEMPT = 1
                    }
                }
            }
