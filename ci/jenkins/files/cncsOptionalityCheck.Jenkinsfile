...
pipeline {
    ...
    parameters {
        string(name: 'INT_CHART_VERSION',
                description: 'The version of the base baseline helmfile to examine' )
    ...
    environment {
        INT_CHART_NAME = "base-platform-baseline"
    ...
    stages {
        ...
        stage('Assess CNCS Optionality'){
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: env.FUNCTIONAL_USER_SECRET, usernameVariable: 'FUNCTIONAL_USER_USERNAME', passwordVariable: 'FUNCTIONAL_USER_PASSWORD')]) {
                        sh "${bob} run-cncs-optionality-check"
                    }
                }
            }
        }
        ...
        always {
            script {
                if (getContext(hudson.FilePath)) {
                    archiveArtifacts artifacts: 'missingOptionalityValues.txt', allowEmptyArchive: true, fingerprint: true
                }
            }
        }
        failure {
