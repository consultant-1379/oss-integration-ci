...
pipeline {
    ...
    stages {
        ...
        stage('Generate CSAR Properties') {
            steps {
                sh '''
                    NEXUS_PATH="https://arm.seli.gic.ericsson.se/artifactory/proj-eric-oss-drop-generic-local/csars"
                    echo "CSAR Built from HELM FILE DETAILS" > artifact.properties
                    echo "helmfile=${INT_CHART_NAME}" >> artifact.properties
                    echo "helmfile_version=${INT_CHART_VERSION}" >> artifact.properties
                    echo "helmfile_repo=${INT_CHART_REPO}" >> artifact.properties
                    echo "" >> artifact.properties
                    echo "CSARS" >> artifact.properties
                    content=$( cat csar_to_be_built.properties )
                    for item in ${content}; do
                        application_name=$( echo ${item} | sed 's/:/ /' | awk '{print $1}' )
                        application_version=$( echo ${item} | sed 's/:/ /' | awk '{print $2}' )
                        echo "${application_name}=${NEXUS_PATH}/${application_name}/${application_version}" >> artifact.properties
                    done
                '''
            }
        }
