...
pipeline {
    ...
    parameters {
        string(name: 'HELMFILE_CHART_VERSION', description: 'The version of the Helmfile sent in through a previous jenkins build\'s artifact.properties.')
    ...
    stages {
        ...
        stage('Compare Application Versions in the Helmfile') {
            steps {
                withCredentials([usernamePassword(credentialsId: env.FUNCTIONAL_USER_SECRET, usernameVariable: 'FUNCTIONAL_USER_USERNAME', passwordVariable: 'FUNCTIONAL_USER_PASSWORD')]) {
                    sh "${bob} compare-component-versions:compare-application-versions-in-helmfile"
                }
