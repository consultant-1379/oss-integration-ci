...
pipeline {
    ...
    parameters {
        ...
        string(name: 'CREATE_TICKETS',
                defaultValue: 'True',
                description: 'A boolean value to indicate whether tickets should be created or not')
        string(name: 'SKIP_LIST',
                defaultValue: 'None',
                description: 'An optional comma-separated list of applications to skip when creating tickets (e.g., eric-cloud-native-base,eric-oss-adc)')
        string(name: 'MICROSERVICE_SKIP_LIST',
                defaultValue: 'None',
                description: 'An optional comma-separated list of microservices to skip when creating tickets (e.g., eric-data-document-database-pg)')
    ...
    stages {
        ...
        stage('Get outdated images and create tickets') {
            steps {
                withCredentials([usernamePassword(credentialsId: env.FUNCTIONAL_USER_SECRET, usernameVariable: 'FUNCTIONAL_USER_USERNAME', passwordVariable: 'FUNCTIONAL_USER_PASSWORD')]) {
                    sh "${bob} create-outdated-images-tickets"
                }
            }
        }
    }
        ...
        always {
            script {
                if (getContext(hudson.FilePath)) {
                    archiveArtifacts artifacts: 'helmfile_shared_images.json, outdated_images_per_chart.json, outdated-ticket-file.txt', allowEmptyArchive: true, fingerprint: true
                }
            }
        }
        failure {
