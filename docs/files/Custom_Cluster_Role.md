# AM Specific Cluster Role Binding Creation.

[TOC]

## Introduction

This file can be used to create a custom cluster role.
The Jenkins files covers the steps from the IDUN & EO Deployment document, section "Pre-Deployment Steps for Using a Custom Cluster Role".
...
This is used when the value global.createClusterRoles is set to false in the site values file.
...
- The file will prepare the file, PrivilegedPolicyClusterRole.yaml and update the necessary section with the
namespace specified as a parameter to the job.
...
- Next it will apply the PrivilegedPolicyClusterRole.yaml if necessary and ensure the cluster role has been created
successfully.
...
- ci/jenkins/files/pre-deploy/custom-cluster-role.Jenkinsfile *(Main Jenkins File)*
- ci/jenkins/rulesets/ruleset2.0.yaml

### Resources

The following is an example of the Jenkinsfile used in a job within the Base and Product staging flows
- [Jenkins Jobs](https://fem7s11-eiffel216.eiffel.gic.ericsson.se:8443/jenkins/job/OSS-Integration-predeploy-custom-cluster-role/)
...
| RELEASE_NAME             | Release name to be added to the file example <meta.helm.sh/release-name: <RELEASE_NAME>-<NAMESPACE_NAME>'                                                                                                                                                      | eric-oss-common-base                                                     |
...
- Within the "Pipeline Section" of the Jenkins Job Configuration set the following
    ...
    * **Script Path:** ci/jenkins/files/pre-deploy/custom-cluster-role.Jenkinsfile
