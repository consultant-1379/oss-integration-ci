...
This file is used to completely clean down the namespace and all it associated resources.
...
- Firstly, the file will remove all the helm release it finds under the namespace using the "helm list" command.
  It uses the --no-hooks to clean down the releases, (helm uninstall --no-hooks <RELEASE>).
  This way it doesn't wait for hooks to gracefully clean-down, and instead just wipes the helm installations from the namespace.

- The script next ensures all the PVC's, TLS secrets and Network Policies are removed and ensures the Deployment Namespace has been removed.

- If CLEANUP_TYPE is set to FULL (Please see the Parameter Table down below), the script will ensure CRD helm releases and CRD components are removed and will ensure the CRD namespace has been deleted.


### Repo Files
The following files within the oss-integration-ci [repo](https://gerrit-gamma.gic.ericsson.se/#/admin/projects/OSS/com.ericsson.oss.aeonic/oss-integration-ci)
are used in its execution.
- ci/jenkins/files/purge.Jenkinsfile *(Main Jenkins File)*
- ci/jenkins/rulesets/ruleset2.0.yaml
- ci/jenkins/scripts/check_remaining_pvs.sh

### Resources

The following is an example of the Jenkinsfile used in a job within the Base and Product staging flows
- [Jenkins Jobs](https://fem5s11-eiffel052.eiffel.gic.ericsson.se:8443/jenkins/job/Purge-Release-in-Namespace/)
...
| CRD_NAMESPACE            | Namespace to purge CRD environment                                                                                                                                                                                                                                                                                                                               | eric-crd-ns                                                              |
| CLEANUP_TYPE             | The Type of cleanup that needs to be done. Selecting FULL will cleanup deployment helm releases, TLS secrets, Network Policies, Installed PVCs, Deployment namespace, CRD helm releases, CRD components and CRD namespace. Selecting PARTIAL will only cleanup deployment helm releases, TLS secrets, Network Policies, Installed PVCs and Deployment Namespace. | PARTIAL                                                                  |
